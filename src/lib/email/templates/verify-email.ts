...
export function verifyEmailTemplate({ name, verifyUrl, appUrl, ttlHours }: Args) {
  ...
  const body = `
    ...
    ${renderSoftPanel(`
      ...
      <p style="margin:12px 0 0 0;font-size:13px;color:${COLORS.textMuted};text-align:center;">This link will expire in ${ttlHours} hours.</p>
    `)}
    <p style="margin:20px 0 0 0;font-size:13px;color:${COLORS.textMuted};">If you did not create a Kalanjiyam account, you can safely ignore this email.</p>
  `;
